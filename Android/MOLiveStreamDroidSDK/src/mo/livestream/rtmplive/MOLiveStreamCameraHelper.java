...
public class MOLiveStreamCameraHelper implements Runnable, Camera.PreviewCallback {
	...
	public void onPreviewFrame(final byte[] data, final Camera camera) {
		...
		if (data == null) {
			...
			if (mMeidaPushHelper.GetMediaLiveStatus() == 1 && mMeidaPushHelper != null) {
				...
				//new PreviewTask(data, camera).execute((Void)null);
				mMeidaPushHelper.OnCaptureVideoFrame(data, data.length, getCameraFace());
	...
	private class PreviewTask extends AsyncTask<Void, Void, Void> {
		private byte[] mData;
		private Camera mCamera;

		PreviewTask(byte[] data, Camera camera) {
			this.mData = data;
			this.mCamera = camera;
		}
		...
		protected Void doInBackground(Void... params) {
			// TODO Auto-generated method stub
			mMeidaPushHelper.OnCaptureVideoFrame(mData, mData.length, getCameraFace());
			return null;
		}
